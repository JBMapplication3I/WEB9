 ...
 */
 ...
 ;(function ($, window, document) {
	...
	var defaults = {
		maxWidth              : 600,      // Max slider width - should be set to image width
		...
		arrowTemplate         : '<a href="#" class="rs-prev"></a><a href="#" class="rs-next"></a>', // String: The markup used for arrow controls (if arrows are used). Must use classes '.rs-next' & '.rs-prev'
	...
	RS.prototype = {
		...
		,setup: function () {
			...
			if (this.settings.usePagination) {
				this.setPagination();
			}

			if (this.settings.useDragging) {
				touchEventsPresent = 'ontouchstart' in window;
				...
				$(document)
					.on(mousemove, function (e) {
						var positionDelta,
							draggingObject = that.$sliderBG.data('dragging');

						if (typeof draggingObject != 'object')
							return;
						...
						if (Math.abs((draggingObject.startPosition.top - e.pageY)/(e.pageX - draggingObject.startPosition.left)) >= 1)
							return;

						positionDelta = draggingObject.startPosition.left - e.pageX;

						if (Math.abs(positionDelta) < 50)
							return false;

						that.$sliderBG.data('dragging', false);
		...
		,setArrows:function () {
			...
			this.$sliderBG.append(this.settings.arrowTemplate);

			// Fire next() method when clicked
			this.$arrowNext = $('> .rs-next', this.$sliderBG).on('click', function (e) {
				e.preventDefault();
				that.next();
			});

			// Fire prev() method when clicked
			this.$arrowPrev = $('> .rs-prev', this.$sliderBG).on('click', function (e) {
				e.preventDefault();
				that.prev();
			});
		}
		...
		,setPagination: function () {
			...
			this.$pagination = $('<ul class="rs-pagination" />')
				.appendTo(this.$sliderWrap);

			for (var i = 0; i < this.totalSlides; i++) {
				this.$pagination.append('<li><p><span>' + (i + 1) + '</span></p></li>');
			}

			this.$paginationItems = $('> li', this.$pagination)
				.on('click', function () {
					that.transition($(this).index());
				});

			this.$paginationItems
				.eq(this.settings.startSlide)
				.addClass('active');
		}
	...
	$.fn['sizeShifter'] = function (settings) {
		return this.each(function () {
			// Check if already instantiated on this elem
			if (!$.data(this, 'sizeShifter')) {
				// Instantiate & store elem + string
				$.data(this, 'sizeShifter', new RS(this, settings));
			}
		});
	}
