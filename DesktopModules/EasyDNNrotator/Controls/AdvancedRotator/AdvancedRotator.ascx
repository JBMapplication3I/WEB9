		...
		<div class="slider_shadows_wrapper <%=rm.ShadowCss%> <%=rm.ShadowStyle%>">
			<div id="container<%=ModuleId%>" <%=ChameleonMCInlineStyle%>>
				<div class="<%=ThemeName%>">
					<div class="screen">
						<noscript>
							<img src="<%=ControlPath.Replace("/Controls/AdvancedRotator","")%>Themes/AdvancedRotator/Main_rotator/assets/loader.gif" alt="" />
						</noscript>
					</div>
						...
						<ul>
							<asp:Repeater ID="repRotator" runat="server" EnableViewState="false">
								<ItemTemplate>
									<li>
										<div class="thumb">
											<img src="<%=PathToImage%>thumb/thumb_<%#(supportsWebp?System.IO.Path.GetFileNameWithoutExtension(Eval("FileSystemName").ToString()) + ".webp":Eval("FileSystemName")) %>" alt="" width="<%=rm.ThumbW%>" height="<%=rm.ThumbH%>" />
											<p>
												<span class="title">
										...
										<a href="<%=PathToImage%><%#(supportsWebp?System.IO.Path.GetFileNameWithoutExtension(Eval("FileSystemName").ToString()) + ".webp":Eval("FileSystemName"))%>"></a><a href="<%#Eval("Link")%>" target="_self"></a>
