; (function ($) {
    $.fn.wtListRotator = function (params) {
        ...
        function VertStripes(rotator, areaWidth, areaHeight, stripeSize, bgColor, duration, delay) {
            ...
            var setVertPieces = function ($img, topPos, opacity, width, alt) {
                ...
                for (var i = 0; i < total; i++) {
                    ...
                    $($stripes.get(i)).css({
                        background: bgColor + " url('" + imgSrc + "') no-repeat", backgroundPosition: xPos + "px " + tOffset + "px",
        ...
        function HorzStripes(rotator, areaWidth, areaHeight, stripeSize, bgColor, duration, delay) {
            ...
            var setHorzPieces = function ($img, leftPos, opacity, height, alt) {
                ...
                for (var i = 0; i < total; i++) {
                    ...
                    $($stripes.get(i)).css({
                        background: bgColor + " url('" + imgSrc + "') no-repeat", backgroundPosition: lOffset + "px " + yPos + "px",
            ...
            var setBlocks = function ($img, opacity, size, tPos) {
                ...
                for (var i = 0; i < numRows; i++) {
                    for (var j = 0; j < numCols; j++) {
                        ...
                        $blockArr[i][j].css({
                            background: bgColor + " url('" + imgSrc + "') no-repeat", backgroundPosition: lVal + "px " + tVal + "px",
        ...
        function ListRotator($obj, opts) {
            ...
            //init rotator
            this.init = function () {
                $rotator = $obj.find("> ." + themeVar); // ovo je izmjenjeno
            ...
            var expandText = function (text, data, props1, props2) {
                ...
                $textBox.stop(true, true).css({ display: "block", top: data.y, left: data.x, "margin-top": 0, "margin-left": 0 }).css(props1).animate(props2, TEXT_SPEED,
                    function () {
                        $innerText.html(text);
                    });
            }
        ...
        var defaults = {
            ...
            vstripe_delay: 90,
            hstripe_delay: 180,
            themeVar: "Main_rotator"	// ovo je dodano
        };
