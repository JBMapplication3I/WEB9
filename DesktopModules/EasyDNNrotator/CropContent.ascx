	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function(){
			...
			jQuery('.send_dataMain').click(function () {
				cropzoomMain.send('<%=ControlPath%>ImageService.ashx', 'POST', { UserId: "<%=UserId %>", ModuleId: "<%=ModuleId %>", ID: 11, ContentID: "<%=ViewState["ContentID"] %>", ImageName: '<%=ViewState["hfImageUNameEdit"] %>', source: '<%=ViewState["source"] %>' },function (imgRet){
			...
			jQuery('.send_dataThumb').click(function () {
				cropzoomThumb.send('<%=ControlPath%>ImageService.ashx', 'POST', { UserId: "<%=UserId %>", ModuleId: "<%=ModuleId %>", ID: 12, ContentID: "<%=ViewState["ContentID"] %>", ImageName: "thumb_<%=ViewState["hfImageUNameEdit"] %>", isThumb: true, source: "<%=ViewState["source"] %>" }, function (imgRet) {
