...
(function (dnnsfAngular15) {
    ...
    function shallowCopy(src, dst) {
        if (dnnsfAngular15.isArray(src)) {
            dst = dst || [];

            for (var i = 0; i < src.length; i++) {
                dst[i] = src[i];
            }
        } else if (dnnsfAngular15.isObject(src)) {
            dst = dst || {};
    ...
    function defaultEquality(a, b, $scope) {
        ...
        return dnnsfAngular15.equals(a, b);
    }
    ...
    dnnsfAngular15.module('treeControl', [])
        .constant('treeConfig', {
            templateUrl: null
        })
        .directive('treecontrol', ['$compile', '$http', function ($compile, $http) {
            /**
            ...
            return {
                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', '$http', function ($scope, $templateCache, $interpolate, treeConfig, $http) {
                    ...
                    $scope.selectNodeHead = function (selectedNode) {
                        ...
                        $http({ method: 'GET', url: requestUrl }).then(function (result) {
                    ...
                    $scope.expandedAllNodes = function (data) { //and select value if any
                        dnnsfAngular15.forEach(data, function (node) {
                    ...
                    $scope.filterTree = function (sTerm) {
                        $http({ method: 'GET', url: $scope.getItems + '&fieldId=' + $scope.fieldId + '&searchText' + $scope.fieldId + '=' + sTerm }).then(function (result) {
                    ...
                    var rootTemplate = '<div class="input-group col-md-12 stylish-input-group" style="padding: 10px;" id="adv-search">' +
                ...
                compile: function (element, attrs, childTranscludeFn) {
                    return function (scope, element, attrs, treemodelCntr) {
                        ...
                        scope.$watch("treeModel", function updateNodeOnRootScope(newValue) {
                            if (dnnsfAngular15.isArray(newValue)) {
                                if (dnnsfAngular15.isDefined(scope.node) && dnnsfAngular15.equals(scope.node[scope.options.nodeChildren], newValue))
                                    return;
                                scope.node = {};
                                ...
                                if (dnnsfAngular15.equals(scope.node, newValue))
                                    return;
                                scope.node = newValue;
                            }
                            ...
                            if (scope.inited == false && scope.initVal) {
                                scope.inited = true;
                                $http({ method: 'GET', url: scope.getItems + '&fieldId=' + scope.fieldId + '&initiallySelected' + scope.fieldId + '=' + scope.initVal }).then(function (result) {
                                    scope.treeModel = result.data;
                                    scope.expandedNodes = [];
                                    scope.selectedNode = [];
                                    scope.expandedAllNodes(result.data);
                        ...
                        scope.$watchCollection('expandedNodes', function (newValue, oldValue) {
                            ...
                            dnnsfAngular15.forEach($liElements, function (liElement) {
                                var $liElement = dnnsfAngular15.element(liElement);
                                var liScope = {
                            ...
                            dnnsfAngular15.forEach(newValue, function (newExNode) {
                                var found = false;
            ...
            return {
                ...
                link: function (scope, element, attrs, controller) {
                    if (!scope.options.isLeaf(scope.node, scope)) {
                        dnnsfAngular15.forEach(scope.expandedNodesMap, function (node, id) {
