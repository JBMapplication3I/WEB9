                ...
                link: {
                    pre: function (scope, element, attr) {
                        ...
                        function reCompileDirective(moduleName) {
                            ...
                            var content = angular.element(element).contents();
                            var sc = angular.element(content).scope();
                            var limitInterval = 30;
                            var checkIfCompiled = $interval(function () {
                                $timeout(function () {
                                    var clonedElement = $compile(content[0])(scope, function (clonedElement, scope) {
                                        if ($(element)) {
                                            $(element).replaceWith(clonedElement);//attach the clone to DOM document at the right place
                                        }
                                        else {
                                            $compile(content[0])(sc)
                                        }
                                    });
