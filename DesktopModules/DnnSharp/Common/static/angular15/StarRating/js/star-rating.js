...
(function (dnnsfjQuery) {
    ...
    Rating.prototype = {
        ...
        _parseAttr: function (vattr, options) {
            ...
            if ($el.attr('type') === 'range' || $el.attr('type') === 'number') {
                var val = validateAttr($el, vattr, options), chk, _final;
                switch (vattr) {
                    case 'min':
                        chk = DEFAULT_MIN;
                        break;
        ...
        clear: function () {
            ...
            self.$element.attr('value', self.clearValue).attr('data-val', self.clearValue);
            self.setStars();
    ...
    dnnsfjQuery.fn.rating = function (option) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                $this.data('rating',
                    (data = new Rating(this, $.extend({}, dnnsfjQuery.fn.rating.defaults, options, $(this).data()))));
            }
    ...
    dnnsfjQuery.fn.rating.defaults = {
        stars: 5,
        glyphicon: true,
        symbol: null,
        ratingClass: '',
    ...
    dnnsfjQuery.fn.rating.Constructor = Rating;

    /**
     * Convert automatically inputs with class 'rating'
     * into the star rating control.
     */
    dnnsfjQuery('input.rating').addClass('rating-loading');

    dnnsfjQuery(document).ready(function () {
        var $input = dnnsfjQuery('input.rating'), count = Object.keys($input).length;
        if (count > 0) {
            $input.rating();
        }
    });
