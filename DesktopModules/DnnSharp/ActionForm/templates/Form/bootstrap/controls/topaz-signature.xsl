...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="ctl-topaz-signature">
    ...
    <script type="text/javascript">
      ...
      <![CDATA[
      //we're using DnnSharpGetSigImageB642 instead of GetSigImageB642
...
$(document).ready(function() {
    ...
    formRoot[0].onFormSubmit.push(function() {
            var numberOfTabletPoints = NumberOfTabletPoints();
            var isRequired = ]]>
      <xsl:choose>
        <xsl:when test="/Form/Settings/ClientSideValidation ='True' and IsRequired='True' and CanValidate = 'True'">
          1;
        </xsl:when>
        <xsl:otherwise>
          0;
      ...
      <![CDATA[
            if (isRequired == 1 && numberOfTabletPoints <= 0) {
      ...
      <xsl:text>';</xsl:text>       <![CDATA[   
                SetImagePenWidth(5); 
                SetImageXSize(c.width()); SetImageYSize(c.height());
                var topazCanvas = $('#cnvTopaz')[0];
                DnnSharpGetSigImageB642(topazCanvas, function(str) {
                  formRoot.find('[name='+fieldName+']').val(str);
           })
    }
});

function DnnSharpGetSigImageB642(cvs, callback) {
...
function GetSigImageB64(callback) {
    ...
    */
    ...
    $.ajax({
        async: false,
        type: "GET",
        url: baseUri + "SigImage/1",
        success: function(data) {
            var blob = new Blob([data]);
            var cntx = cvs.getContext('2d');
            var img = new Image();
            img.src = setTimeout(getBlobURL(blob), 2000);
            img.onload = function() {
              cntx.drawImage(img, 0, 0);
              var b64String = cvs.toDataURL("image/png");
              var loc = b64String.search("base64,");
