...
(function (angular) {
    ...
    function FormCtrl($scope, $http, sharedData, $timeout, $sce, dnnsf, dnnsfHttp) {
        ...
        $scope.actionDefGroupsnotFlushable = dnnsf.configUtils.groupDefinitions($scope.actionDefs,
            function (actiondef) { return actiondef.Settings.IsFlushableAction != 'true'; });
        ...
        $scope.prepareFields = function (fields) {
            ...
            $.each(fields, function (i, field) {
                ...
                field.$_uid = dnnsf.uniqueId('field', field.FormFieldId);
                field.AutoName = !field.Name;
                field.BoundName = field.BoundName || field.Name;
                $scope.computeName(field);

                // parse server validators
                if (field.CustomValidator1 && $scope.validatorDefs[field.CustomValidator1] && $scope.validatorDefs[field.CustomValidator1].IsServerSideValidation)
                    $scope.sharedData.serverValidators.push({ title: field.Title + ' Field: ' + $scope.validatorDefs[field.CustomValidator1].Title + ' Failed', value: field.FormFieldId + '-' + $scope.validatorDefs[field.CustomValidator1].Title });
                if (field.CustomValidator2 && $scope.validatorDefs[field.CustomValidator2] && $scope.validatorDefs[field.CustomValidator2].IsServerSideValidation)
                    $scope.sharedData.serverValidators.push({ title: field.Title + ' Field: ' + $scope.validatorDefs[field.CustomValidator2].Title + ' Failed', value: field.FormFieldId + '-' + $scope.validatorDefs[field.CustomValidator2].Title });
                if (field.ValidationGroup && $scope.groupValidatorDefs[field.GroupValidator] && $scope.groupValidatorDefs[field.GroupValidator].IsServerSideValidation)
                    $scope.sharedData.serverValidators.push({ title: field.ValidationGroup + ': ' + $scope.groupValidatorDefs[field.GroupValidator].Title + ' Failed', value: field.ValidationGroup + '-' + $scope.groupValidatorDefs[field.GroupValidator].Title });

                dnnsf.initParameters($scope.fieldDefs[field.InputTypeStr], field, true);
            });
        ...
        $scope.cloneField = function (field, predefField, forExport) {
            ...
            if (!forExport) {
                newField.RowIndex = $scope.highestRow() + 1;
                newField.ColSpan = 12;
                newField.ColOffset = 0;
            }
                ...
                'buttons': [
                    {
                        ...
                        'onClick': function ($event, item) {
                            var fields = JSON.parse(item.Parameters.Fields);
                            fields = cleanImportFields(fields);
                            sharedData.form.Fields = sharedData.form.Fields.concat(fields);
        ...
        function cleanImportFields(fields) {
            ...
            var lastRow = $scope.highestRow();
            if (lastRow > 0) lastRow++;
            var currentImportIndex = fields[0].RowIndex;
            var currentRowTotalColumns = 0;

            return fields.map(function (fld) {
                currentRowTotalColumns += fld.ColOffset + fld.ColSpan;
                if (currentImportIndex !== fld.RowIndex || currentRowTotalColumns > 12) {
                    currentImportIndex = fld.RowIndex;
                    lastRow++;
                    currentRowTotalColumns = fld.ColOffset + fld.ColSpan;
                }
                fld.RowIndex = lastRow;
                return fld;
            });
        }
